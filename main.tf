...
module "vpc" {
  ...
  version            = "5.9.0"
  name               = coalesce(each.value.vpc_name, local.aws_network_settings.main-vpc.vpc_name)
  cidr               = coalesce(each.value.vpc_cidr, local.aws_network_settings.main-vpc.vpc_cidr)
  azs                = coalesce(each.value.azs, local.aws_network_settings.main-vpc.azs)
  private_subnets    = coalesce(each.value.private_subnets, local.aws_network_settings.main-vpc.private_subnets)
  public_subnets     = coalesce(each.value.public_subnets, local.aws_network_settings.main-vpc.public_subnets)
  enable_nat_gateway = coalesce(each.value.enable_nat_gateway, local.aws_network_settings.main-vpc.enable_nat_gateway)
  enable_vpn_gateway = coalesce(each.value.enable_vpn_gateway, local.aws_network_settings.main-vpc.enable_vpn_gateway)
  tags               = coalesce(each.value.tags, local.aws_network_settings.main-vpc.tags)
}
